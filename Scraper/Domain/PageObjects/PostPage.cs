...
namespace Instagram_Scraper.Domain.PageObjects
{
    public class PostPage
    {
        ...
        private IWebElement MultiSrcPostChevron => 
            _webHelper.SafeFindElement(".coreSpriteRightChevron");

        private IWebElement NextPostPaginationArrow => 
            _webHelper.SafeFindElement(".coreSpriteRightPaginationArrow");
        

        private IEnumerable<IWebElement> ImageSourceClass => 
            _webHelper.SafeFindElements(".FFVAD");

        private IEnumerable<IWebElement> VideoSourceClass => 
            _webHelper.SafeFindElements(".tWeCl");
        
        
        private IWebElement ViewAllCommentsClass => 
            _webHelper.SafeFindElement(".lnrre");
        
        private IEnumerable<IWebElement> CommentUserList => 
            _webHelper.SafeFindElements(".FPmhX").ToList();
        
        private IEnumerable<IWebElement> CommentTextList => 
            _webHelper.SafeFindElements("div.C4VMK span").ToList();
        ...
        private void GetComments(string timeStamp)
        {
            ViewAllCommentsClass?.Click();

            var commentUserList = CommentUserList
                .Select(username => username.GetAttribute("title"))
                .ToList();
            var commentTextList = CommentTextList
            ...
            var zippedList = commentUserList
                .Select((t, i) => new KeyValuePair<string, string>(t, commentTextList[i]))
                .ToList();
